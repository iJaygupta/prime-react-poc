...
export class FileUpload extends Component {
  ...
  validate(file) {
    if (this.props.maxFileSize && file.size > this.props.maxFileSize) {
      this.messagesUI.show({
        severity: "error",
        summary: this.props.invalidFileSizeMessageSummary.replace(
          "{0}",
          file.name
        ),
        detail: this.props.invalidFileSizeMessageDetail.replace(
          "{0}",
          this.formatSize(this.props.maxFileSize)
        )
      });
  ...
  renderAdvanced() {
    ...
    if (this.hasFiles()) {
      ...
      progressBar = (
        <ProgressBar value={this.state.progress} showValue={false} />
      );
    }
